...
-- Load the notification module by asset ID
local MODULE_ID = "rbxassetid://106961888365668"
local moduleInstance = game:GetObjects(MODULE_ID)[1]
local Notifications = require(moduleInstance)
local notify = Notifications.notify  -- replaces old notify
...
local function isSandboxed()
    ...
    local success = pcall(function()
        env.HttpService:RequestAsync({Url = "https://google.com", Method = "GET"})
    end)
    if not success then
        table.insert(flags, "Http blocked or faked")
    end

    local badGlobals = {"is_sirhurt_closure", "pebc_execute", "syn", "secure_call", "getrenv",
...
local success, res = pcall(function()
    return env.HttpService:RequestAsync({
        ...
        Body = env.HttpService:JSONEncode({tempKey = tempKey, username = detectedUsername})
    })
end)
